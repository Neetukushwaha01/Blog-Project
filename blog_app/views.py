...
# only data entry code its dont ues
def create_dummy_blogs(request):
    user = User.objects.filter(username='admin').first()  # change if needed

    if not user:
        return JsonResponse({'error': 'User not found'}, status=404)

    last_blog = BlogPost.objects.filter(title__startswith="Test Blog").order_by('-id').first()
    start_num = 1

    if last_blog:
        try:
            # extract last number
            last_number = int(last_blog.title.split()[-1])
            start_num = last_number + 1
        except:
            pass
    for i in range(start_num, start_num + 10):
        BlogPost.objects.create(
            title=f"Test Blog {i}",
            content=f"This is the content of blog post number {i}.",
            status='draft',
            author=user
        )

    return JsonResponse({'message': f'Created blog posts: Test Blog {start_num} to Test Blog {start_num + 9}'})